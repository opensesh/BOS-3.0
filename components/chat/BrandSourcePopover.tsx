    ...
    <span className="flex items-start gap-2.5 px-2.5 py-2 rounded-lg hover:bg-[var(--bg-primary)]/50 transition-all duration-200 group cursor-pointer">
      ...
      <span className="flex-shrink-0">
        {isAsset && isImage && source.thumbnail ? (
          <span className="w-8 h-8 rounded-lg overflow-hidden bg-[var(--bg-primary)]/50 block">
            <Image
          ...
          <span className="w-8 h-8 rounded-lg bg-[var(--bg-primary)]/50 flex items-center justify-center">
            <ImageIcon className="w-4 h-4 text-[var(--fg-tertiary)]/60" />
          </span>
        ) : (
          <span className="w-8 h-8 rounded-lg bg-[var(--bg-primary)]/50 flex items-center justify-center">
      ...
      <span className="flex-1 min-w-0 flex flex-col pt-0.5">
        <span className="text-[13px] font-medium text-[var(--fg-primary)] group-hover:text-[var(--fg-primary)] transition-colors line-clamp-1">
          {source.title || source.name}
        </span>
        <span className="text-[11px] text-[var(--fg-tertiary)]/50 mt-0.5">
          Internal knowledge
        </span>
      </span>
      ...
      <span className="flex-shrink-0 flex items-center">
        {isAsset ? (
          <button
            ...
            className="p-1.5 rounded-md opacity-0 group-hover:opacity-100 hover:bg-[var(--bg-primary)]/50 transition-all"
            title="Copy path"
          >
            {copied ? (
