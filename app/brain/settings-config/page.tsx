...
export default function SettingsConfigPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/claude/settings.json')
      .then(res => {
        if (!res.ok) throw new Error('File not found');
        return res.text();
      })
      .then(text => {
        // Wrap JSON in code fence for syntax highlighting
        const formattedContent = '```json\n' + text + '\n```';
        setContent(formattedContent);
      })
      .catch(err => {
        console.error('Failed to load settings.json:', err);
        setContent('# Settings\n\nSettings configuration not yet available. Create `.claude/settings.json` to display content here.');
      });
  }, []);
        ...
        <PageTransition className="w-full max-w-6xl mx-auto px-6 py-8 md:px-12 md:py-12">
          ...
          <MotionItem className="flex flex-col gap-2 mb-10">
            ...
            <p className="text-base md:text-lg text-[var(--fg-tertiary)] max-w-2xl">
              MCP server permissions and Claude Code preferences. Controls which external tools
              and services Claude can access when working in this project.
            </p>
          </MotionItem>
