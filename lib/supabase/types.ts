...
export function dbShortLinkToApp(db: DbShortLink): ShortLink {
  const shortUrl = `https://${db.domain}/l/${db.short_code}`;

  // Build full destination URL with UTM params
  let fullDestinationUrl = db.destination_url;
  const utmParams = new URLSearchParams();
  if (db.utm_source) utmParams.set('utm_source', db.utm_source);
  if (db.utm_medium) utmParams.set('utm_medium', db.utm_medium);
  if (db.utm_campaign) utmParams.set('utm_campaign', db.utm_campaign);
  if (db.utm_term) utmParams.set('utm_term', db.utm_term);
  if (db.utm_content) utmParams.set('utm_content', db.utm_content);
  if (utmParams.toString()) {
    const separator = db.destination_url.includes('?') ? '&' : '?';
    fullDestinationUrl = `${db.destination_url}${separator}${utmParams.toString()}`;
  }
  ...
  return {
    ...
    domain: db.domain,
    destinationUrl: db.destination_url,
    title: db.title || undefined,
    description: db.description || undefined,
    tags: db.tags || [],
    hasPassword: !!db.password_hash,
    expiresAt: db.expires_at || undefined,
    utmSource: db.utm_source || undefined,
    utmMedium: db.utm_medium || undefined,
    utmCampaign: db.utm_campaign || undefined,
    utmTerm: db.utm_term || undefined,
    utmContent: db.utm_content || undefined,
    clicks: db.clicks,
    uniqueClicks: db.unique_clicks,
    lastClickedAt: db.last_clicked_at || undefined,
...
// ============================================
// SHORT LINK CLICKS (Analytics)
// ============================================

export type DeviceType = 'mobile' | 'tablet' | 'desktop';

export interface DbShortLinkClick {
  id: string;
...
export interface ShortLinkClickInsert {
  short_link_id: string;
  ip_address?: string;
  country?: string;
  country_code?: string;
...
export function dbShortLinkClickToApp(db: DbShortLinkClick): ShortLinkClick {
  return {
    id: db.id,
    shortLinkId: db.short_link_id,
    clickedAt: db.clicked_at,
    ...
    browser: db.browser || undefined,
    browserVersion: db.browser_version || undefined,
    os: db.os || undefined,
    osVersion: db.os_version || undefined,
    deviceType: db.device_type || undefined,
    deviceBrand: db.device_brand || undefined,
    referer: db.referer || undefined,
    refererDomain: db.referer_domain || undefined,
    utmSource: db.utm_source || undefined,
    utmMedium: db.utm_medium || undefined,
    utmCampaign: db.utm_campaign || undefined,
  };
...
export interface DbShortLinkTag {
  id: string;
  brand_id: string;
  name: string;
  slug: string;
...
export interface ShortLinkTagInsert {
  brand_id: string;
  name: string;
  slug?: string;
  color?: ShortLinkTagColor;
...
export interface ShortLinkTagUpdate {
  name?: string;
  slug?: string;
  color?: ShortLinkTagColor;
  usage_count?: number;
...
export function dbShortLinkTagToApp(db: DbShortLinkTag): ShortLinkTag {
  return {
    id: db.id,
    brandId: db.brand_id,
    name: db.name,
...
// ============================================
// SHORT LINK ANALYTICS AGGREGATES
// ============================================
